#ifndef DOMAINS_DISPLAY_SCREENS_SOME_OTHER_SCREEN_HPP
#define DOMAINS_DISPLAY_SCREENS_SOME_OTHER_SCREEN_HPP
...
class SomeOtherScreen {
    // These are available for derived classes
    static constexpr char *TAG = "SomeOtherScreen";
    BaseScreen *base = nullptr;

    public:

    SomeOtherScreen(BaseScreen * _base): base(_base) {
        assert(base);
    }

// Destructor - proper cleanup of LVGL objects
    virtual ~SomeOtherScreen()
    {
    ...
    void init() {
        ...
        lv_label_set_text(label, "Some Other Page");                  // Set label text
